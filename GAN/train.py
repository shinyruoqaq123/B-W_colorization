...
if args.d_init is not None:
    print("load pretrained D from {}".format(args.d_init))
    D.load_state_dict(torch.load(args.d_init,map_location='cuda:0'))
if args.g_init is not None:
    print("load pretrained G from {}".format(args.g_init))
...
for epoch in range(args.start_epoch,args.epoch):
    for y, uv in training_generator:
        ...
        i+=1
        if i%10==0:
            print("Epoch: %d, batch: %d: [D loss: %f] [G total loss: %f] [G GAN Loss: %f]" % (
